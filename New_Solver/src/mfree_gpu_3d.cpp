...
float_x global_substrate_velocity_y;
float_x global_substrate_velocity_z;
...
int main(int argc, char *argv[])
{
	...
	std::cout << "Terminal test\n";
	std::cout << "Code is running\n";

	const std::string logFileName = "simulation.log";

	{
		std::ofstream clearFile(logFileName, std::ios::trunc);
		if (!clearFile.is_open())
		{
			std::cerr << "Error: could not clear the Simulation log" << std::endl;
			return 1;
		}
	}

	std::ofstream lfile(logFileName, std::ios::app);

	if (!lfile.is_open())
	{
		std::cerr << "Error: could not open the Simulation log" << std::endl;
		return 1;
	}

	lfile << "Simulation started" << std::endl;
	...
	if (!file.is_open())
	{
		...
		lfile << "JSON file could not be opened / found!" << std::endl
		<< "The name of the file has to be: FS_values.json" << std::endl;

		std::cout << "JSON file could not be opened / found!" << std::endl << "The name of the file has to be: FS_values.json" << std::endl;
	...
	which_GPU = j["GPU"]["GPU_id"].get<int>();

	std::string gpu_str = std::to_string(which_GPU);

	// Set CUDA_VISIBLE_DEVICES to select the third GPU
	if (setenv("CUDA_VISIBLE_DEVICES", gpu_str.c_str(), 1) != 0)
	{
		lfile << "Failed to set CUDA_VISIBLE_DEVICES" << std::endl;
		return -1;
	}
	lfile << "CUDA_VISIBLE_DEVICES set to " << which_GPU << std::endl;
	...
	lfile << "Either define thermal conduction in types.h via Thermal_Conduction_Brookshaw OR Thermal_Conduction_PSE, but not both!" << std::endl;
	exit(-1);
#endif
	...
	particle_gpu *particles = setup_FS_with_json_HaBe(80, &grid);

	int device_id = 0; // After setting CUDA_VISIBLE_DEVICES, the selected GPU is indexed as 0
	cudaError_t err = cudaSetDevice(device_id);
	if (err != cudaSuccess)
	{
		lfile << "Failed to set device " << device_id << ": " << cudaGetErrorString(err) << std::endl;
		return -1;
	}
	lfile << "Using GPU " << device_id << std::endl;
...
#ifdef USE_DOUBLE
	lfile << "this executable uses DOUBLE precision!" << std::endl;
#else
	lfile << "this executable uses SINGLE precision!" << std::endl;
#endif

#ifdef CSPM
	lfile << "using CSPM" << std::endl;
#endif

#ifdef Thermal_Conduction_PSE
	lfile << "thermal conduction enabled using PSE" << std::endl;
#endif

#ifdef Thermal_Conduction_Brookshaw
	lfile << "thermal conduction enabled using Brookshaw approximation" << std::endl;
#endif
	...
	for (int step = 0; step < last_step; step++)
	{
		...
		if (step == 1000)
		{
			...
			lfile << "estimated time for the thermo-meachnical solver: " << time_left << " seconds, which is " << time_left / 60. / 60. << " hours" << std::endl;
		...
		if (record_this_step)
		{
			...
			lfile << step << " of " << last_step << ": "
				  << percent_done << " percent done, "
				  << seconds_so_far << " sec of est runtime "
				  << time_left / 3600.0 << " hr, at temp "
				  << temp << std::endl;
	...
	lfile << "gpu time: (seconds) " << milliseconds / 1e3 << std::endl;

	// printf("gpu time: (seconds) %f\n", milliseconds / 1e3);

	lfile << "Simulation done" << std::endl;
